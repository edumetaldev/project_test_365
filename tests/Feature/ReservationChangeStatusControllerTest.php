...
final class ReservationChangeStatusControllerTest extends TestCase
{
    ...
    {
        $response = $this->patchJson("/api/reservations/{$this->reservation->id}/status", [
            'status' => 'CONFIRMED'
        ]);
    ...
    public function test_fails_when_reservation_does_not_exist(): void
    {
        ...
        $response = $this->patchJson("/api/reservations/{$nonExistentId}/status", [
            'status' => 'CONFIRMED'
        ]);
    ...
    {
        return [
            'lowercase_pending' => ['pending'],
            'lowercase_confirmed' => ['confirmed'],
